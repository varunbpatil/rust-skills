...
Cargo runs a build script when it considers that package dirty. If the script
emits no rerun directives, Cargo's default is to scan the package and rerun when
any package file changes; precise `cargo::rerun-if-changed` and
`cargo::rerun-if-env-changed` directives narrow that set. Overly broad
directives cause unnecessary rebuilds, while incomplete ones can leave stale
generated output. Non-determinism (timestamps, UUIDs, network fetches) breaks
...
capability probes against the actual toolchain.
...
fn main() {
    // With no rerun directives, any changed package file can rerun this.
